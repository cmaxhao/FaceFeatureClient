    ...
    {/// <summary>
     ...
     /// <param name="port">端口号</param>
        ...
        public override void OnPost(HttpRequest request, HttpResponse response)
        {
            ...
            else if (data.Equals("stop") && mainForm != null)
            {
                state = 1;
                mainForm.StopClick();
                //发送响应
                buffer = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(new { success = "true", msg = "摄像头关闭成功" }));
                response.SetContent(buffer);
                response.Send();
        ...
        public override void OnGet(HttpRequest request, HttpResponse response)
        {
            ...
            if (mainForm == null) {
                mainForm = new MainForm(session, realName);
                state = 1;
                mainForm.response = response;
            }
            if (session != null&&state!=2)
            {
                state = 2;
